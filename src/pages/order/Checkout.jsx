...
const Checkout = () => {
  ...
  // Identify pet vs product
  const isPet = item.breed && item.age;

  const endpoint = isPet ? "pets" : "products";
  const itemId = item.productId;
console.log("Updating stock for:", item.name, "| ID:", itemId);
  await axios.patch(`${BASE_URL}/${endpoint}/${itemId}`, {
    stock: updatedStock,
  });
}
...
<div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-6 shadow-lg transform transition-all duration-300 hover:shadow-xl animate-fade-in-up">
  <div className="flex items-center gap-3 mb-6">
    ...
    <div>
      <h3 className="text-xl font-bold text-gray-800">Order Bill</h3>
  ...
  <div className="space-y-3 mb-4">
    {cartItems.map((item, index) => (
      <div key={item.id} className="flex justify-between items-center py-2 border-b border-orange-200 last:border-b-0">
      ...
      {showConfirm && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 backdrop-blur-sm">
          <div className="bg-white p-8 rounded-2xl shadow-2xl text-center max-w-md mx-4 transform transition-all duration-300 scale-100 animate-modal-enter">
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-4">Confirm Your Order</h3>
